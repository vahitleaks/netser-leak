...
/*       Rapor gönderilecek mail adreslerini çek       */    
     ...
     $sql_mail = "SELECT * FROM DEPTS";
     if (!($cdb->execute_sql($sql_mail, $res_mail, $error_msg))){
       print_error($error_msg);
       exit;
    }
    while($rw_mail = mysql_fetch_object($res_mail)){
      $email = $rw_mail->DEPT_RSP_EMAIL;
      $SITE_ID = $rw_mail->SITE_ID;
      if($email==""){continue;}

      $DEPT_ID = $rw_mail->DEPT_ID;
          ...
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                ...
                <TD width=\"50%\" align=center CLASS=\"header\"><br>".get_site_name($SITE_ID)."<BR>$report_name<BR>$header_text<BR><STRONG>Dönem</STRONG> : ".strftime("%B", mktime(0, 0, 0, $mymonth-1 ,32, $myyear))." ".$myyear."</TD>
       ...
       $origCrt = get_dept_extnos($DEPT_ID, $SITE_ID);
       $kriter = $kriter." AND ORIG_DN IN ".$origCrt." "; 
      ...
      if(mysql_num_rows($result)==0){
        ...
        while($row = mysql_fetch_object($result)){
          ...
          $DATA .= $rows_header;
          $DATA .= " <td class=\"rep_td\">".number_format($OUT_TOTAL_AMOUNT,0,'','.')."</td>\n
          <td class=\"rep_td\">".calculate_all_time($OUT_TOTAL_DUR)."</td>\n
          <td class=\"rep_td\">".write_price($OUT_TOTAL_PRICE)."</td>\n
          <td class=\"rep_td\">".number_format($INB_TOTAL_AMOUNT,0,'','.')."</td>\n
          <td class=\"rep_td\">".calculate_all_time($INB_TOTAL_DUR)."</td>\n
          <td class=\"rep_td\">".number_format($INTERNAL_TOTAL_AMOUNT,0,'','.')."</td>\n
          <td class=\"rep_td\">".calculate_all_time($INTERNAL_TOTAL_DUR)."</td>\n
          <td class=\"rep_td\">".number_format($EXT_TOTAL_AMOUNT,0,'','.')."</td>\n
          <td class=\"rep_td\">".calculate_all_time($EXT_TOTAL_DUR)."</td>\n
          <td class=\"rep_td\">".write_price($EXT_TOTAL_PRICE)."</td>\n
          <td class=\"rep_td\">".number_format($EXT_AVG_AMOUNT,0,'','.')."</td>\n
          <td class=\"rep_td\">".calculate_all_time($EXT_AVG_DUR)."</td>\n
          <td class=\"rep_td\">".write_price($EXT_AVG_PRICE)."</td>\n
          </tr>\n";
        }
      }
