...
//*********************//
//BU DEÐERLER FORTISTE EKLENEN "Yurtdýþý - VoIP" VE "SES NETWORKU" PROVIDERLARININ 
//TEL_PROVIDER_ID DEÐERLERÝ OLACAKTIR (OKOC)
     $INT_VOIP_VAL = 11;
     $VOICE_NW_VAL = 11;
//*********************//
...
<?
    //Joinden kaçmak için extentions tablosundaki bilgiler alýnýyor.
    $sql_str="SELECT EXT_NO,DESCRIPTION,ACCOUNT_NO FROM EXTENTIONS WHERE SITE_ID=".$SITE_ID; 
    ...
    while ($row = mysql_fetch_object($result)){
        $arr_ext_nos[$row->EXT_NO][0] = $row->ACCOUNT_NO;
        $arr_ext_nos[$row->EXT_NO][1] = $row->DESCRIPTION;
    }
   ...
   if ($act == "src") {
    ...
    $sql_str  = "SELECT CDR_MAIN_DATA.LocationTypeid AS TYPE, CDR_MAIN_DATA.FROM_PROVIDER_ID AS FROM_ID, LTRIM(CDR_MAIN_DATA.ORIG_DN) AS ORIG_DN, SUM(CDR_MAIN_DATA.$myfld) AS TOTAL
                 FROM $CDR_MAIN_DATA AS CDR_MAIN_DATA
                 ";

      if ($kriter != "")
            $sql_str .= " WHERE ".$kriter." ".$usr_crt;
     $sql_str .= " GROUP BY ORIG_DN, FROM_ID, TYPE";
      ...
      while($row = mysql_fetch_object($result)){  
            ...
            if(($DEPT_ID && $DEPT_ID != '-1') || $D_ID){
                if($my_dept == $DEPT_ID){
                    ...
                    }else if($row->FROM_ID == $INT_VOIP_VAL && $row->TYPE == 3){
                        $datas[$my_dept][$row->ORIG_DN][3] += $row->TOTAL;
                    }else if($row->FROM_ID == 4 || $row->FROM_ID == 5){
                        $datas[$my_dept][$row->ORIG_DN][4] += $row->TOTAL;
                    }else if($row->FROM_ID == 2){
                        $datas[$my_dept][$row->ORIG_DN][5] += $row->TOTAL;
                    }else if($row->FROM_ID == 3){
                        $datas[$my_dept][$row->ORIG_DN][6] += $row->TOTAL;
                    }else if($row->FROM_ID == $VOICE_NW_VAL && $row->TYPE != 3){
                        $datas[$my_dept][$row->ORIG_DN][7] += $row->TOTAL;
                    }else{
                        $datas[$my_dept][$row->ORIG_DN][8] += $row->TOTAL;
                    }
                    $datas[$my_dept][$row->ORIG_DN][10] += $row->TOTAL;
                }
            }else{
                if($row->ORIG_DN != ""){
                    $datas[$my_dept][$row->ORIG_DN][9]=1;
                    if($row->FROM_ID == 1){
                      if($row->TYPE == 0){
                    ...
                    }else if($row->FROM_ID == $INT_VOIP_VAL){
                        $datas[$my_dept][$row->ORIG_DN][3] += $row->TOTAL;
                    }else if($row->FROM_ID == 4 || $row->FROM_ID == 5){
                        $datas[$my_dept][$row->ORIG_DN][4] += $row->TOTAL;
                    }else if($row->FROM_ID == 2){
                        ...
                        $datas[$my_dept][$row->ORIG_DN][6] += $row->TOTAL;
                    }else if($row->FROM_ID == $VOICE_NW_VAL){
                        $datas[$my_dept][$row->ORIG_DN][7] += $row->TOTAL;
                    }else{
                        $datas[$my_dept][$row->ORIG_DN][8] += $row->TOTAL;
                    }
                    ...
                    if($row->FROM_ID == 1){
                      if($row->TYPE == 0){
                        ...
                        $datas[0][0][5] += $row->TOTAL;
                    }else if($row->FROM_ID == 3){
                        $datas[0][0][6] += $row->TOTAL;
                    }else if($row->FROM_ID == $VOICE_NW_VAL){
                        $datas[0][0][7] += $row->TOTAL;
                    }else{
                        $datas[0][0][8] += $row->TOTAL;
                    }
                    $datas[0][0][10] += $row->TOTAL;
                }
            }
        }
if (is_array($datas)){
  ...
  <tr>
    <td colspan="2">
      <table width="100%" border="0" bgcolor="#C7C7C7" cellspacing="1" cellpadding="0">
          ...
          <tr>
<?if($CSV_EXPORT==2){?>
                <td class="rep_table_header" width="28%">Sicil</td>
                <td class="rep_table_header" width="28%">Ýsim</td>
                <td class="rep_table_header" width="28%">Dahili</td>
                <td class="rep_table_header" width="12%">Þehir Ýçi(TT)</td>
                <td class="rep_table_header" width="14%">Þehirler Arasý(TT)</td>
                <td class="rep_table_header" width="14%">Yurt Dýþý(TT)</td>
                <td class="rep_table_header" width="14%">Yurt Dýþý(VOIP)</td>
                <td class="rep_table_header" width="12%">AVEA</td>
                <td class="rep_table_header" width="12%">TURKCELL</td>
                <td class="rep_table_header" width="12%">VODAFONE</td>
                <td class="rep_table_header" width="12%">Ses Network</td>
...
<?}else{?>
                ...
                <td class="rep_table_header" width="12%">Þehir Ýçi(TT)<a style="cursor:hand;" onclick="javascript:submit_form('0');"><img src="<?=IMAGE_ROOT?><?=($order=="0")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="14%">Þehirler Arasý(TT)<a style="cursor:hand;" onclick="javascript:submit_form('1');"><img src="<?=IMAGE_ROOT?><?=($order=="1")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">Yurt Dýþý(TT)<a style="cursor:hand;" onclick="javascript:submit_form('2');"><img src="<?=IMAGE_ROOT?><?=($order=="2")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">Yurt Dýþý(VOIP)<a style="cursor:hand;" onclick="javascript:submit_form('3');"><img src="<?=IMAGE_ROOT?><?=($order=="3")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">AVEA<a style="cursor:hand;" onclick="javascript:submit_form('4');"><img src="<?=IMAGE_ROOT?><?=($order=="4")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">TURKCELL<a style="cursor:hand;" onclick="javascript:submit_form('5');"><img src="<?=IMAGE_ROOT?><?=($order=="5")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">VODAFONE<a style="cursor:hand;" onclick="javascript:submit_form('6');"><img src="<?=IMAGE_ROOT?><?=($order=="6")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">Ses Network<a style="cursor:hand;" onclick="javascript:submit_form('7');"><img src="<?=IMAGE_ROOT?><?=($order=="7")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="10%">Diðer <a style="cursor:hand;" onclick="javascript:submit_form('8');"><img src="<?=IMAGE_ROOT?><?=($order=="8")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
                <td class="rep_table_header" width="12%">Toplam<a style="cursor:hand;" onclick="javascript:submit_form('10');"><img src="<?=IMAGE_ROOT?><?=($order=="10")?$sort_gif:"sort.gif"?>" align="absmiddle" Title=""></a></td>
<?}?>
          </tr>
        <tr>
      ...
      <? 
        ...
        $my_pr=0;
                ...
                $csv_data[0][1] = "Ýsim";
                $csv_data[0][2] = "Dahili";
                $csv_data[0][3] = "Þehir Ýçi";
                $csv_data[0][4] = "Þehirler Arasý" ;
                $csv_data[0][5] = "GSM";
                $csv_data[0][6] = "Uluslar Arasý";
                $csv_data[0][7] = "Diðer";
                $csv_data[0][8] = "Toplam";
         ...
         if(is_array($datas)){    
            ...
            echo  " <tr  bgcolor=\"FFFFFF\"><td class=\"rep_td\" colspan=13>&nbsp;<b>".$my_dept_name."</b></td></tr>\n";
            unset($dept_totals);
            $i = 0;
            $j++;
            ...
            foreach($value as $keynew=>$valuenew){
                 ...
                 echo " <tr  BGCOLOR=$bg_color>\n";
                   ...
                   if($CSV_EXPORT==2){
                     echo $arr_ext_nos[$keynew][0]."</td>\n";
                     echo  " <td class=\"rep_td\">&nbsp; ".$arr_ext_nos[$keynew][1]."</td> <td class=\"rep_td\">&nbsp; <b>".$k_x."</b></td>\n";
                   }else{
                     echo " &nbsp;".$arr_ext_nos[$keynew][0]."</td> <td class=\"rep_td\">&nbsp; ".$arr_ext_nos[$keynew][1]."</td>\n";
                     echo  "<td class=\"rep_td\"><a class=\"a1\" HREF=\"javascript:drill_down('$k_y')\"><b>".$k_x."</b></a></td> \n";
                   }  
                   $total = 0;
             $csv_data[$j][0] =  $arr_ext_nos[$keynew][0];
             $csv_data[$j][1] =  $arr_ext_nos[$keynew][1];
             $csv_data[$j][2] =  $k_x;
                   for($k=0;$k<=8;$k++){
                       echo " <td class=\"rep_td\" align=\"right\">".write_me($value[$keynew][$k],$calc_type)."</td>\n";
                       $total += $value[$keynew][$k];
                       $csv_data[$j][$k+3] =  write_me($value[$keynew][$k],$calc_type);
            ...
            $csv_data[$j][1] = "";         
            $csv_data[$j][2] = "";         
            echo  " <tr bgcolor=\"E4E4E4\"><td class=\"rep_td\" colspan=3>&nbsp;<b>Alt Toplamlar</b></td>\n";
               for($k=0;$k<=9;$k++){
                   echo " <td class=\"rep_td\" align=\"right\"><b>".write_me($dept_totals[$k],$calc_type)."</b></td>\n";
               $csv_data[$j][$k+3] =  write_me($dept_totals[$k],$calc_type);
               }
            echo "</tr>";   

            echo  " <tr bgcolor=\"000000\"><td colspan=13 height=\"1\" align=center valign=center></td></tr>\n";   
      }
          }  
          $j++;
      ?>
