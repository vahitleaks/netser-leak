...
function sendAttMail($email, $page){
  ...
  $iYear = substr($t0, 0, 4);
      ...
      $Mail->Body = "Bu e-posta,Fortis santral raporlama sistemi tarafýndan otomatik olarak gönderilmektedir. 
			Raporlama ile ilgili soru ve sorunlarýnýz için http://yardim ekranýndan SES DESTEK'e çaðrý açabilirsiniz.
      ...
      $Mail->IsSMTP();
              $strAlert = "";
      if (!$Mail->Send()){
                       $strAlert = "Mail \" $email \" adresine gönderilemedi, Hata:". $Mail->ErrorInfo."\n";
          echo "Mail \" $email \" adresine gönderilemedi, Hata:". $Mail->ErrorInfo."\n";
          return;
      }
      echo "Mail baþarýyla \" $email \" adresine gönderilmiþtir\n";
             $strAlert ="Mail baþarýyla \" $email \" adresine gönderilmiþtir\n";
          $sql_str1= "insert into TbMailingLog (StMailAddress, StNotes, DtDateTime) 
	              values('".$email."', '".$strAlert."', now());";
    if (!($cdb->execute_sql($sql_str1,$result1,$error_msg))){
        echo $error_msg;
    }
}
