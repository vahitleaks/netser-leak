       ...
       <form name="auth_code_src" method="post" action="auth_code_src.php?act=src">
         ...
         <table cellpadding="0" cellspacing="0" align="center" width="75%">
            ...
            <tr class="formbg" id="dept_select"> 
                <td width="45%" class="font_beyaz" style="width:200">Departman</td>
                <td>
            ...
            <tr class="form">
                  <td class="font_beyaz">Meridian Mail</td>
                     <td><input type="checkbox" class="input1" NAME="MER_MAIL" <?if ($MER_MAIL==1){echo 'checked';}?> value="1"></td>
            </tr>
            <tr class="form">
                  <td class="font_beyaz">Ayrýntýlý Döküm</td>
                     <td><input type="checkbox" class="input1" NAME="DETAIL" <?if ($DETAIL==1){echo 'checked';}?> value="1"></td>
            </tr>   
            <tr> 
...
<?
   ...
   if ($act == "src") {
         ...
         $kriter .= $cdb->field_query($kriter, "DETAIL",     "=", "'$DETAIL'");
         $kriter .= $cdb->field_query($kriter, "MER_MAIL",     "=", "'$MER_MAIL'");
                        
         $sql_str  = "SELECT AUTH_CODES.AUTH_CODE_ID,AUTH_CODES.AUTH_CODE,AUTH_CODES.AUTH_CODE_DESC,
                        AUTH_CODES.DETAIL,AUTH_CODES.MER_MAIL,SITES.SITE_NAME,DEPTS.DEPT_NAME
                      FROM AUTH_CODES
                      INNER JOIN SITES ON AUTH_CODES.SITE_ID = SITES.SITE_ID
                      LEFT JOIN DEPTS ON AUTH_CODES.DEP_ID = DEPTS.DEPT_ID
                      ";
         if ($kriter != "")
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr class="header_beyaz">
        ...
        <td><a href="javascript:submit_form('auth_code_src',1, 'DEPTS.DEPT_NAME')">Departman</a></td>        
        <td><a href="javascript:submit_form('auth_code_src',1, 'AUTH_CODES.AUTH_CODE')">Auth.Kodu</a></td>
        <td><a href="javascript:submit_form('auth_code_src',1, 'MER_MAIL')">Meridian Mail</a></td>
        <td><a href="javascript:submit_form('auth_code_src',1, 'DETAIL')">Detay istiyor</a></td>
   ...
   while($row = mysql_fetch_object($rs)){
    ...
    echo " <tr class=\"".bgc($i)."\">".CR;
        echo " <td height=\"20\">$row->AUTH_CODE_ID</td> ".CR;
         ...
         echo " <td>".YesOrNo($row->MER_MAIL)."</td>".CR;
         echo " <td>".YesOrNo($row->DETAIL)."</td>".CR;
