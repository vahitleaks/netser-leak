<?php
    ...
    $p = 0;$x=0;
//    ECHO sizeof($res)."--".$SITE_ID;DIE;
    while($x<sizeof($res)){
//    ECHO $res[$x][0][1];
          if($res[$x][0][1]==$SITE_ID){
            $data[] = $res[$x][1][1];
            if($res[$x][1][1]>=1000000 && $multp<=1000000){
              $multp=1000000;
            }elseif($res[$x][1][1]>=100000 && $multp<=100000){
              $multp=100000;
            }elseif($res[$x][1][1]>=10000 && $multp<=10000){
              $multp=10000;
            }elseif($res[$x][1][1]>=1000 && $multp<=1000){
              $multp=1000;
            }else{
              $multp=1;
            }
    ...
    $c = new XYChart(419, 282, 0xF0F0E8, 0x0, 1);
    //$c = new XYChart(430, 257, "", Transparent);
...
//  $legendObj->setBackground(0x9cbade, -1, 0xffffff);
    ...
    if($multp>1)
      $yTitle = "Tutar (x".number_format($multp, 0, ',', '.').")";
    else
      $yTitle = "Tutar";
    $c->yAxis->setTitle(turkish2utf($yTitle));
