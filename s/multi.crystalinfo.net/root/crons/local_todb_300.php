...
*/
$local_root = "/home/mahmutbeykb/";  //BUFFER IN DOSYALARI KOYDUÐU YER
...
while($list[$i]) { 
          ...
          if (file_exists($local_root.$file_name)) {
                    ...
                    $handle2= fopen("/usr/local/sigma/crystal/data/".date("m")."_".date("Y")."/300raw_".date("m")."_".date("Y").".dat","a+");
                    if ($handle) {
                              while (!feof($handle)) {
                                        ...
                                        if(strlen($buffer)>2){
                                                  ...
                                                  $sqlQuery =  "\n INSERT INTO RAW_DATA(DATA, DATE, SOURCE, SITE_ID,ERROR_CODE) VALUES ('".$buffer."',CURDATE(),'buffer',300, '0')"; 
	...
	global $local_root;
	 if ($handle = opendir($dir)) {
          ...
          while (false !== ($file = readdir($handle))) {
               ...
               if ($file != "." && $file != "..")  {
                  if(substr($file,0, 4)=="raw_"){ // uygun dosya 
                    $array[] = $file;
