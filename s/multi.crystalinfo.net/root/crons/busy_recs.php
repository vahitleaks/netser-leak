...
require_once("doc_root.cnf");
  ...
  */      
  ...
  while($row = mysql_fetch_object($result)){
    ...
    //////////////////////////////////////////////////////////////
    $sql1 = "SELECT CDR_MAIN_DATA.SITE_ID, CDR_ID, EXTENTIONS.EMAIL, TIME_STAMP, CLID, TER_DN, EXTENTIONS.DESCRIPTION AS DES, E.DESCRIPTION as EDES FROM CDR_MAIN_INB AS CDR_MAIN_DATA 
    ...
    while($row1 = mysql_fetch_object($rs1)){
      ...
      $DATA = "<b> $row1->CLID - $row1->EDES</b> numaralý telefon <b> $row1->TIME_STAMP </b>  tarihinde sizi( Dahili :<b> $row1->TER_DN   $row1->DES</b>) aradý, ulaþamadý.<BR><BR><BR>";
      $DATA .= "You( Extention :<b> $row1->TER_DN  $row1->DES</b>) called by <b>$row1->CLID </b> at <b> $row1->TIME_STAMP </b> but the number couldn't reach  ";
