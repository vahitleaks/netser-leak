  ...
  while($row = mysql_fetch_object($rs)){//Bütün siteler taranmalý Site döngüsü start
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapýlacakýr. Mail atýlacak mý start
      if($row->MONTHLY_MAILING_DAY == date("d") || $force==1){//Mail günü geldi mi start
        ...
        <center>  
        <br><br>
        Geçen aya ait telefon görüþme detayýnýz ektedir. <br>Bilgi için baþvuru adresi : teknik@kardemir.com.tr<br>
        $stnotes
        <table width=\"65%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                <TD width=\"100%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylýk Dahili Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
              </TR>
            </TABLE>
            </td>
          </tr>
        ...
        //$QRY = "SELECT * FROM EXTENTIONS WHERE (EMAIL<>'' AND INSTR(EMAIL,'@')) AND SITE_ID = ".$SITE_ID;
          if(strlen($sicilnocrt)>0){
            $sicilnocrt=" AND SICIL_NO='".$sicilnocrt."'  ";
          }
          ...
          if($unsentmails=="1"){
            $QRY = "select EXTENTIONS.SICIL_NO, EXTENTIONS.EMAIL from EXTENTIONS  
                    ...
                    WHERE (EMAIL<>'' AND INSTR(EMAIL,'@')) AND TMAILLOGS.ID IS NULL ".$sicilnocrt." 
                    group by SICIL_NO,EMAIL  
                    ORDER BY SICIL_NO ";
          }else{
           $QRY = "select SICIL_NO, EMAIL from EXTENTIONS  WHERE (EMAIL<>'' AND INSTR(EMAIL,'@'))".$sicilnocrt." group by SICIL_NO, EMAIL ORDER BY SICIL_NO";
          }
...
//          $orig_dn = ;
          ...
          $kriter .= $cdb->field_query($kriter,   "CDR_MAIN_DATA.ACCESS_CODE" ,  "IN",  "('01', '09', '0', '011045')");
