  ...
  while($row = mysql_fetch_object($rs)){//Bütün siteler taranmalý Site döngüsü start
    ...
    if($row->MONTHLY_MAILING_DAY > '0'){//0'dan büyükse ilgili günde mailing yapýlacakýr. Mail atýlacak mý start
      if($row->MONTHLY_MAILING_DAY == date("d") || $force==1){//Mail günü geldi mi start
        $report_name = "Aylýk Top 100 Görüþme Raporu";
        $DATA_HEADER ="
        <html>
        <head>
          ...
          <tr>
            ...
            <TABLE BORDER=\"0\" WIDTH=\"100%\">
              <TR>
                ...
                <TD width=\"50%\" align=center CLASS=\"header\">".$SITE_NAME . "<BR>Aylýk Top 100 Görüþme Raporu<br><STRONG>Dönem</STRONG> : <!-- DONEM --></TD>
        ...
        $DATA = "";
          if ($ADMIN_EMAIL<>"" || $SITE_ADMIN<>""){//Mail adresi yoksa göndermenin bir anlamý yok Email var mý start
            $sql_str = "SELECT LTRIM(ORIG_DN) AS ORIG_DN, 
                        ...
                        ";
            $sql_str .= " WHERE ".$kriter."  ORDER BY PRICE DESC LIMIT 100 ";
            //echo $sql_str;exit;
            ...
            $my_pr=0;
            while($row_data = mysql_fetch_object($result_data)){//Rapor içeriði start
              if($row_data->ORIG_DN != ""){//Orig alaný dolumu start
                ...
                $DATA  .= " <td class=\"rep_td\" align=\"center\">".write_price($row_data->PRICE * $PRICE_FACTOR)."</td>\n";
                $DATA  .= "</tr>\n";
                $my_pr = $my_pr + $row_data->PRICE * $PRICE_FACTOR;
            ...
            if($DATA !=""){
              ...
              if($prnscreen==1){
                ...
                if(strlen($sendadres)){
    			  mail_send($sendadres,"Top 100  Görüþme Raporu -- $SITE_NAME",$DATA);
                }else{
			      mail_send($SITE_ADMIN,"Top 100  Görüþme Raporu -- $SITE_NAME",$DATA);
			      mail_send($ADMIN_EMAIL,"Top 100  Görüþme Raporu -- $SITE_NAME",$DATA);
                }
              }
