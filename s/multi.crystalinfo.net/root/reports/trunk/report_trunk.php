<?
  ...
  //Hak kontrolü sonu  
    ...
    page_header();
?>
<form name="trunk_search" method="post" action="report_general_prn.php?act=src">
<table cellpadding="0" cellspacing="0" align="center" border="0" width="98%">
  ...
  <tr>
    ...
    <?table_header("Hat Raporlarý","100%");?>
    <center>
      <table cellpadding="0" cellspacing="0" align="center" border="0" width="100%">
      <tr height="22"> 
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="trunk_outb" onclick="javascript:set_me('trunk_outb')"></td>      
        <td width="90%" class="td1_acik" id="trunk_outb"><b>Giden Çaðrýlarýn</b> Hatlara Göre Daðýlýmý</td>
      </tr>
      <tr height="22"> 
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="trunk_inb" onclick="javascript:set_me('trunk_inb')"></td>      
        <td width="90%" class="td1_acik" id="trunk_inb"><b>Gelen Çaðrýlarýn</b> Hatlara Göre Daðýlýmý</td>
      </tr>
      <tr height="22"> 
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="trunk_det" onclick="javascript:set_me('trunk_det')"></td>      
        <td width="90%" class="td1_acik" id="trunk_det">Hatlardaki <b>Çaðrýlarýn Türlerine</b> Göre Daðýlýmý</td>
      </tr>
      <tr height="22"> 
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="matrix_pr" onclick="javascript:set_me('matrix_pr')"></td>      
        <td width="90%" class="td1_acik" id="matrix_pr">Operatör <b>Þebeke Matrisi</b></td>
      </tr>
      <tr height="22">
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="trunk_prc" onclick="javascript:set_me('trunk_prc')"></td>      
        <td width="90%" class="td1_acik" id="trunk_prc">Hatlarýn <b>Kullaným Yüzdeleri</b></td>
             </tr>
      <tr height="22">
        <td width="10%"><input type="radio" class="input1" name="trunk_rep" value="trunk_act" onclick="javascript:set_me('trunk_act')"></td>      
        <td width="90%" class="td1_acik" id="trunk_act">Hat <b>Aktivite</b> Raporu</td>
             </tr>
      </table>
    <?table_footer();?>
  </td>
  ...
  <tr>
      <td colspan=4 align=center><br>
           <a href="javascript:submit_form('trunk_search');"><img name="Image631" border="0" src="<?=IMAGE_ROOT?>raporal.gif"></a>
         </td>
     </tr>            
</table>    
   </form>
   <a href="report_trunk.php"><img border="0" src="<?=IMAGE_ROOT?>kriter_temizle.gif"></a>
<?page_footer(0);?>
...
//Time functions ends here
  ...
  function submit_form(){
    ...
    if (mytype=='matrix_pr'){
       ...
       document.all('trunk_search').target= 'report_screen';
       document.all('trunk_search').action = '/reports/general/report_matrix_prn.php?act=src&type=' + mytype;
       document.all('trunk_search').submit();
    }else if (mytype=='trunk_prc'){
       popup('','report_screen',800,600)
       document.all('trunk_search').target= 'report_screen';
       document.all('trunk_search').action = 'report_trn_prn.php?act=src&type=' + mytype;
       document.all('trunk_search').submit();
    }else if (mytype=='trunk_act'){
            popup('','report_screen',800,600)
            document.all('trunk_search').target= 'report_screen';
      document.all('trunk_search').action = 'report_trn_act.php?act=src&type=' + mytype;
          document.all('trunk_search').submit();
    }else if (mytype=='trunk_det'){
            popup('','report_screen',800,600)
            document.all('trunk_search').target= 'report_screen';
      document.all('trunk_search').action = 'report_call_trunk.php?act=src&type=' + mytype;
          document.all('trunk_search').submit();
    }else{
            popup('','report_screen',800,600)
            document.all('trunk_search').target= 'report_screen';
      document.all('trunk_search').action = 'report_trunk_prn.php?act=src&type=' + mytype;
          document.all('trunk_search').submit();
    }
  }
</script>